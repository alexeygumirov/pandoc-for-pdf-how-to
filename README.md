...
> For my formatting following replacement works: `markdown_github` -> `markdown_strict+pipe_tables+backtick_code_blocks+auto_identifiers`. Below all scripts are given with the new `markdown_strict` source format.

```sh
pandoc -s -o $DEST.pdf -f "markdown_strict+pipe_tables+\
 backtick_code_blocks+auto_identifiers+yaml_metadata_block+\
...
## Unnumbered header of level 4 and lower

Sometimes there is a need to make an unnumbered header of level 4 and lower.
The problem of Pandoc is that it puts line break after unnumbered header of levels 1 to 3, but when header is of level 4 and below, then line break is not added.

And we end up with this ugly situation.

#### Level 4 unnumbered header {-}

Text of chapter below header, but Pandoc does not put any break after it. Let's fix this broken formatting.

#### Level 4 unnumbered header with proper line break {-}
\
